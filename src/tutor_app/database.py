...
DB_PATH = Path(__file__).resolve().parent.parent.parent / "thinkstep.db"
...
class Database:
    ...
    def _init_schema(self) -> None:
        ...
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_step_attempts_session
                ON step_attempts(session_id);
            CREATE INDEX IF NOT EXISTS idx_step_attempts_user
                ON step_attempts(user_id);
        """)
        conn.commit()
    ...
    def get_user_by_username(self, username: str) -> dict[str, Any] | None:
        row = self._conn().execute("SELECT * FROM users WHERE username = ?", (username.lower(),)).fetchone()
        return dict(row) if row else None

    def update_user_age_group(self, user_id: str, age_group: str) -> None:
        self._conn().execute(
            "UPDATE users SET age_group = ?, updated_at = ? WHERE user_id = ?",
